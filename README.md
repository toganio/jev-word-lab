...
Each dimension has six nonexclusive tags. Jev selects one of the 63 nonempty subsets, or N/A or uncertain: **65 options per question**, below 255. Up to 32 independent word/dimension questions share a request. The full 87,776-word dictionary has 3,159,936 cells, requiring about 98,748 successful requests from scratch. This is a large, potentially costly scan, not a quick initialization. The UI displays the remaining request estimate. Only the user's explicit button starts paid calls; file loading never does. The browser must remain open; closing pauses work and resuming requires the key again. No other model is used.
