...
## English interface and Jev continuation correction (2026-09-19)

The interface, errors, category labels and new operation traces are in English. Existing category files remain compatible; presentation labels may differ while IDs, tags, schema, dictionary identity and coverage still match.

Version 10 reduced routing depth using balanced alphabetical ranges and introduced independent yes/no grammar vetoes. Recorded runs showed a quality regression: the stop option was vetoed on every available step in five inspected runs, while ungrammatical continuations were still approved. Some runs consumed the full request budget without finishing. Fewer tree levels did not establish better response quality.

**Reply style** defaults to **Three short sentences**; **One short sentence** is also available. Jev receives the sentence target and completed-sentence count at every step. The application stops after the selected number of terminal punctuation tokens, but Jev can choose to end earlier instead of adding filler. Word and request budgets still apply.

Version 12 restores spelling-prefix routing and fuller group examples, retains all 120 shortlisted base words instead of truncating to 48, and keeps prepared-tree caching. Questions carry the user's request and existing reply explicitly; only the first step is marked as a new answer. No category assignments were changed.

**Jev grammar guidance**, enabled by default, adds candidate inflections and asks Jev to compare grammar and meaning in the same final choice. Independent grammar vetoes have been removed. The end option remains available after the first token; only Jev decides to select it. The application never forces a replacement word or uses another model. This removes one grammar round trip per selected token, although prefix routing can require more steps than range routing.

Inflections are spelling candidates, not new classified entries. Only Jev chooses the emitted form. This remains an experimental dictionary-constrained generator: short outputs can still be incomplete or factually wrong. Regression tests verify candidate coverage, end-option availability, request limits and the absence of substitute answers; they do not prove linguistic quality.
