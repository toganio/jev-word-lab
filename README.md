...
Preparation now fans out independent eight-word jobs across a bounded pool (default ceiling 32 concurrent requests, configurable to 8/16/32). Each request batches up to 128 questions within a conservative 56,000-byte serialized context budget; these are app limits, not documented Jev question-count limits. Each dimension uses two independent three-tag questions with nine options each (seven nonempty subsets, none, uncertain). Both responses travel in one request and are combined into the existing six-tag bitset; all 63 nonempty combinations remain expressible. An uncertain group marks the dimension uncertain. This preserves the file schema and all 36 dimensions while avoiding repeated 65-option lists. A worker owns all dimensions of each assigned word and awaits its own durable checkpoint. Concurrent checkpoints are coalesced for 25 ms into serialized writes of at most 100 words, reducing database round trips. Idempotent save timeouts and temporary storage failures retry without calling Jev again. Existing category files, schema versions and prepared results remain compatible.
